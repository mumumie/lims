(function (global, factory) {
  ...
  if (typeof require === 'function' && typeof module === "object" && module && module["exports"])
    module['exports'] = (function () {
      ...
      var Long = require("./dist/long.min");
      ProtoBuf.util.Long = Long;
      ProtoBuf.configure();
      return factory(ProtoBuf, require('./dist/bytebuffer.min.js'), require('./dist/pako.min.js'));
    })();
  /* Global */
  else
    global["jbuscore"] = factory(global.protobuf, global.dcodeIO.ByteBuffer, global.pako);
...
var Observer = function () {
  function Observer() {
    ...
    this.timeoutCheckerId = setInterval(function () {
      var curTime = new Date().getTime();
      for (var key in _this.subscribers) {
        var fnCount = _this.subscribers[key].length;
        var deleteCount = 0;
        // 检查请求的对象为空或者应答超时
        for (var i = 0; i < fnCount; i++) {
          var fnObj = _this.subscribers[key][i];
          if (fnObj) {
            if (_this.responseTimeout > 0 && curTime - fnObj.time > _this.responseTimeout * 1000) {
  ...
  _createClass(Observer, [{
    ...
    value: function setResponseTimeoutSecond(second) {
    ...
    value: function sub(evt, fn) {
      ...
      var fnNumber = this.subscribers[evt].length - 1;
    ...
    key: 'pub',
    value: function pub(evt, args) {
      if (!this.subscribers[evt]) {
        console.error('put not find, sn:' + evt);
        return;
      }

      for (var i = 0, count = this.subscribers[evt].length; i < count; i++) {
        var fnObj = this.subscribers[evt][i];
...
var jbuscore = function () {
  function jbuscore(cmdParse) {
    _classCallCheck(this, jbuscore);
  ...
  _createClass(jbuscore, [{
    ...
    key: 'setConnectOptions',
    value: function setConnectOptions(opts) {
      for (var p in opts) {
        if (this.settings[p]) {
    ...
    key: 'connect',
    value: function connect(wsurl, opts) {
      var self = this;
      ...
      this.ws.onmessage = function (evt) {
        ...
        for (var i = 0, count = packages.length; i < count; i++) {
          var p = packages[i];
    ...
    key: 'buildProtoPackage',
    value: function buildProtoPackage(proto_package) {
      var _this = this;
    ...
    key: 'buildProtoObject',
    value: function buildProtoObject(proto_package, msg) {
      var _this2 = this;
      return new Promise(function (resolve, reject) {
        return resolve(msg);
      });
    ...
    key: 'requestOnce2',
    value: function requestOnce2(cmd, proto_package, msg, callback) {
      var _this3 = this;
      return this.buildProtoObject(proto_package, msg).then(function (obj)
      {
        // 验证填充的数据是否有效
        let bytes = msg.serializeBinary();
        // 包装成ByteBuffer
        let binary = ByteBuffer.wrap(bytes, "binary");
        return _this3.sendmsg(cmd, binary, proto_package, callback, false);
      }
      );
    }
    ...
    key: 'sendmsg',
    value: function sendmsg(cmd, byteBuffer, proto_package, callback, forever) {
      ...
      if (this.ws && this.ws.readyState === WebSocket.OPEN) {
        if (forever === undefined || !forever) {
          ...
          this.subscribeInfo(PREFIX_DATABUS, serialnum, function(info, command) {
            ...
            let type = self.cmdParse.getType(command);
            let msg;
            if (type && type.deserializeBinary) {
              msg = type.deserializeBinary(info.view);
            }
            if (msg instanceof proto.MsgExpress.ErrMessage) {
              try {
    ...
    key: 'setPushDataFactory',
    value: function setPushDataFactory(factory) {
      this.pushDataFactory = factory;
    }
  }, {
    key: 'subscribeInfo',
    value: function subscribeInfo(prefix, id, callback) {
      var self = this;
    ...
    key: 'register',
    value: function register(apiName,serviceId,serviceName,type) {
      this.cmdParse.register(apiName,serviceId,serviceName,type);
    }
  }, {
    key: 'publishInfo',
    value: function publishInfo(prefix, id, info, command) {
