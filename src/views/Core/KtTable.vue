    ...
    <el-table
      ...
      <el-table-column :label="$t('action.operation')"  :width="btnWidth" fixed="right" v-if="showOperation" header-align="center">
        <template slot-scope="scope">
          <kt-button :label="isStatus(scope.row.status)" :perms="permsEdit" :size="size" @click="handleApprove(scope.$index, scope.row)" v-if="showBtnApprove"/>
          ...
          <kt-button  class="btns" :label="isStatusExam(scope.row)" :perms="permsExam" :size="size" @click="handleExam(scope.$index, scope.row)" v-if="showBtnExam"/>
          <kt-button  class="btns" label="重置" :perms="permsDetail" :size="size" @click="handleReset(scope.$index, scope.row)" v-if="showReset"/>
...
export default {
  ...
  methods: {
    ...
    handleReset:function(index, row){
      this.$emit('handleReset', {index:index, row:row})
    },
