    ...
    <el-table
      ...
      <el-table-column :label="$t('action.operation')"  :width="btnWidth" fixed="right" v-if="showOperation" header-align="center">
        <template slot-scope="scope">
          <kt-button icon="fa fa-edit" :label="$t('action.edit')" :perms="permsEdit" :size="size" :disabled="scope.row.status !== 0" @click="handleEdit(scope.$index, scope.row)" v-if="showBtnEdit"/>
          <kt-button icon="fa fa-trash" :label="$t('action.delete')" :perms="permsDelete" :size="size" :disabled="scope.row.status !== 0" type="danger" @click="handleDelete(scope.$index, scope.row)" v-if="showBtnDelete"/>
          ...
          <kt-button class="btns" :label="$t('action.detail')" :perms="permsDetail"  :size="size" @click="handleDetail(scope.$index, scope.row)" v-if="showBtnDetail"/>
          <kt-button  class="btns" :label="isStatusExam(scope.row)" :perms="permsExam" :disabled="scope.row.status === 2"  :size="size" @click="handleExam(scope.$index, scope.row)" v-if="showBtnExam"/>
          <kt-button class="btns" label="阅卷" :perms="permsEdit" :size="size" :disabled="scope.row.status === 0" @click="handleApprove(scope.$index, scope.row)" v-if="showBtnApprove"/>
        </template>
      </el-table-column>
    </el-table>
    <!--分页栏-->
  ...
  methods: {
    ...
    isStatusExam:function(row){
      if(row.myExamResult && row.myExamResult.status === 1){
        ...
        return '已批阅'
      }else if(row.myExamResult && row.myExamResult.status === 0){
        return '继续考试'
      }else{
        return '考试'
      }
