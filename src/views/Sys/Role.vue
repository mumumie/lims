<template>
  <div class="page-container">
    <tip :value="
    ['用于创建新角色，并赋予角色菜单权限；',
     '只有授权后，该角色下的账户才能使用权限内的功能菜单列表。比如学生角色，授权“练习”菜单权限，则该角色只能使用练习的菜单功能',
     '修改角色拥有的菜单后，该角色下的所有用户在重新登录时将看到新的菜单列表', '选中一个角色，在下方为其指定菜单，然后点击保存']" />
	<!--工具栏-->
...
<!--        <el-select v-model="dataForm.typeNameList" multiple placeholder="请选择">-->
<!--          <el-option-->
...
<!--      </el-form-item>-->
	...
	<div class="menu-container" :v-if="true">
		...
		<div style="float:right;padding-right:15px;padding-top:4px;padding-bottom:4px;">
			...
			<kt-button :label="$t('action.submit')" perms="" type="primary" @click="submitAuthForm"
...
export default {
    ...
    handleDelete: function (params) {
      this.$confirm('确定删除这一项吗？', '提示', {
        ...
        this.$api.role.singleDelete(params.row).then(res =>{
          if (res.retCode === 0) {
            this.$message({
              type:"success",
              message:'已删除！！！'
      ...
      this.dialogVisible = true
		...
		handleRoleSelectChange(val) {
			...
			this.$api.role.findRoleMenus({'roleId':val.val.id}).then((res) => {
				this.currentRoleMenus = res.bean.menuList
				this.$refs.menuTree.setCheckedNodes(res.bean.menuList)
			})
		},
		// 树节点选择监听
