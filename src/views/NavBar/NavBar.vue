<template>
	<div class="menu-bar-container">
    <!-- logo -->
    <div class="logo" :style="{'background-color':themeColor}" :class="collapse?'menu-bar-collapse-width':'menu-bar-width'"
      @click="$router.push('/')">
        <img v-if="collapse" src="@/assets/logo.png"/> <div>{{collapse?'':appName}}</div>
    </div>
    <!-- 导航菜单 -->
    <el-menu ref="navmenu" default-active="1" :class="collapse?'menu-bar-collapse-width':'menu-bar-width'"
      :collapse="collapse" :collapse-transition="false" :unique-opened="true"
      @open="handleopen" @close="handleclose" @select="handleselect">
      <!-- 导航菜单树组件，动态加载菜单 -->
      <menu-tree v-for="item in navTree" :key="item.id" :menu="item"></menu-tree>
    </el-menu>

    <!-- ICP证 -->
    <div class="ipc" :class="collapse?'menu-bar-collapse-width':'menu-bar-width'"
         @click="$router.push('/')">
      <a href="http://www.beian.miit.gov.cn" target="_blank">ICP证：皖ICP备19021738号</a>
    </div>
	</div>
</template>

<script>
import { mapState } from 'vuex'
import MenuTree from "@/components/MenuTree"
export default {
  components:{
    MenuTree
  },
  computed: {
    ...mapState({
      appName: state=>state.app.appName,
      themeColor: state=>state.app.themeColor,
      collapse: state=>state.app.collapse,
      // navTree: state=>state.menu.navTree
    }),
    navTree: {
      get () { return this.$store.state.menu.navTree },
      set (val) { this.$store.commit('setNavTree', val) }
    },
    mainTabs: {
      get () { return this.$store.state.tab.mainTabs },
      set (val) { this.$store.commit('updateMainTabs', val) }
    },
    mainTabsActiveName: {
      get () { return this.$store.state.tab.mainTabsActiveName },
      set (val) { this.$store.commit('updateMainTabsActiveName', val) }
    }
  },
  watch: {
    $route: 'handleRoute'
  },
  created () {
    this.handleRoute(this.$route)
  },
  methods: {
    handleopen() {
      // console.log('handleopen')
    },
    handleclose() {
      // console.log('handleclose')
    },
    handleselect(a, b) {
      // console.log('handleselect')
    },
    getCurriculum:function(){
      let pageRequest={
        pageNum: 1,
        condition:{
          status:0
        }
      };
      let curriculum=0;
      this.$api.curriculum.findPage(pageRequest).then((res) => {
        curriculum = res.bean.total;

      });
      return curriculum;
    },
    getStatus:function(){
      let navTree=this.$store.state.menu.navTree;
      this.navTree=navTree.map( item =>{
        item.children.map( ite =>{
          if (ite.name === '我的课程') {
            // ite.value=this.getCurriculum();
            ite.value=3;
          }else{
            ite.value=0
          }
          return ite;
        });
        return item;
      });

      // this.$store.commit('setNavTree', res.result.menuTree)
    },
    // 路由操作处理
    handleRoute (route) {
      // tab标签页选中, 如果不存在则先添加
      let tab = this.mainTabs.filter(item => item.name === route.name)[0];
      let tabIndex = this.mainTabs.findIndex(item => item.name === route.name);
      // console.log(tab);
      // console.log(tabIndex);
      tab = {
        name: route.name,
        title: route.name,
        icon: route.meta.icon,
        path:route.path,
        params:route.params,
        query:route.query,
      };
      if (tabIndex === -1) {
        this.mainTabs = this.mainTabs.concat(tab)
      }else{
        this.mainTabs[tabIndex] =tab;
      }
      this.mainTabsActiveName = tab.name;
      // 切换标签页时同步更新3高亮菜单
      if(this.$refs.navmenu != null) {
        this.$refs.navmenu.activeIndex = '' + route.meta.index;
        this.$refs.navmenu.initOpenedMenu()
      }
    }
  }
}
</script>

<style scoped lang="scss">
.menu-bar-container {
  position: fixed;
  top: 0px;
  left: 0;
  bottom: 0;
  z-index: 10;
  .el-menu {
    position:absolute;
    top: 60px;
    bottom: 0px;
    text-align: left;
    // background-color: #2968a30c;
  }
  .ipc {
    position:absolute;
    bottom: 0px;
    height: 20px;
    line-height: 20px;
    cursor:pointer;
    a {
      font-size: 4px;
      color: black;
      text-align: left;
      padding-left: 20px;
      text-decoration:none;
    }
  }
  .logo {
    position:absolute;
    top: 0px;
    height: 60px;
    line-height: 60px;
    background: #545c64;
    cursor:pointer;
    img {
        width: 40px;
        height: 40px;
        border-radius: 0px;
        margin: 10px 10px 10px 10px;
        float: left;
    }
    div {
      font-size: 25px;
      color: white;
      text-align: left;
      padding-left: 20px;
    }
  }
  .menu-bar-width {
    width: 200px;
  }
  .menu-bar-collapse-width {
    width: 65px;
  }
}

</style>
