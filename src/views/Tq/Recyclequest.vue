  ...
  export default {
    ...
    methods: {
      ...
      findPage: function (data) {
        ...
        pageRequest.condition.status = 2;
      ...
      handleDelete: function (params) {
        this.$confirm('试题删除后不可恢复，确定删除这一项吗？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$api.quest.singleDelete(params.row).then(res =>{
            if (res.retCode === 0) {
              this.$message({
                type:"success",
                message:'已删除试题！！！'
      ...
      handleBatchDelete: function (params) {
        this.$confirm('试题删除后不可恢复，确定批量删除吗？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$api.quest.deleteBatchBean(params).then(res =>{
            if (res.retCode === 0) {
              this.$message({
                type:"success",
                message:'已删除试题！！！'
