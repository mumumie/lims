<template>
  <div class="page-container">
    ...
    <!--新增编辑界面-->
    <el-dialog :title="operation?'新增':'编辑'" width="50%" :visible.sync="dialogVisible" :close-on-click-modal="false" append-to-body>
      <el-form
        :model="dataForm"
        class="experiment_label"
        label-width="100px"
        :rules="dataFormRules"
        ref="dataForm" :size="size"
        ...
        <el-form-item label="关联实验" prop="expTypeName">
          <el-select
            v-model="dataForm.expTypeName"
            filterable
            placeholder="请选择"
            :disabled="operation?false:true"
            @change="typeNameChange(dataForm.expTypeName,null)"
            style="width: 100%;">
        ...
        <div v-if="dataForm.expTypeName === 'ExpOne222'">
...
<!--      </div>-->
    ...
    <!--导入实验数据-->
    <el-dialog title="导入实验数据" width="60%" :visible.sync="dialogVisibleBatch" :close-on-click-modal="false" append-to-body>
      <div style="margin-bottom:20px;" v-if="hasPerms('teach:curriculum:edit')">
        <el-select
          size="mini"
...
<!--        <a href="/static/批量导入用户模板.xlsx" download="批量导入用户模板.xlsx">下载模板</a>-->
        <el-upload
          ...
          :on-change="importExcel2"
          :file-list="fileList"
          :auto-upload="false">
          <el-button slot="trigger" size="small" type="primary">导入实验数据</el-button>
          <div slot="tip" class="el-upload__tip">只能上传xlsx文件，且不超过500kb</div>
        </el-upload>
      </div>
      <div style="margin-top:15px;" v-for="item in importExcelNav">
        <el-input placeholder="请输入xlsx中对应的名称" v-model="importExcelData[item.name]" size="mini">
          <template slot="prepend">{{item.zhName}}</template>
        </el-input>
      </div>
      <div slot="footer" class="dialog-footer">
...
<script>
  ...
  import {addBean, queryBean, updateBean, addBatchBean,importOriginResultExcel} from "@/http/base";
  ...
  export default {
    ...
    data() {
      return {
        ...
        dataFormRules: {
          ...
          expTypeName :[
            { required: true, message: '请选择关联实验', trigger: 'blur' }
          ]
        },
        // 新增编辑界面数据
        dataForm: {
    ...
    methods: {
      ...
      handleEdit: function (params) {
        ...
        if(this.dataForm.expTypeName){
          let expTypeName = this.dataForm.expTypeName.replace(this.dataForm.expTypeName[0],this.dataForm.expTypeName[0].toLowerCase());
          console.log(expTypeName);
          this.typeNameChange(this.dataForm.expTypeName,params.row[expTypeName]);
        }else{
          this.dataTemplateForm= {
            pj11:[55,65],
              pj11Score:15,
      ...
      handleDetail: function(params) {
        ...
        addBean('ExperimentResult',postData)
          .then(res =>{
            if(res.retCode === 0){
              let id = res.bean.id;
              console.log(id);
              this.$router.push({path:'/teach/experimentdetail',query:{id:id}});
            }
          })
      },
      //导入数据
      handleApprove:function(params){
        ...
        let typeName = params.row.expTypeName +'OriginResult';
        this.$api.data.findTableInfo(typeName)
          .then(res =>{
            let arr=[];
            res.bean.fieldProps.map(item =>{
              if (item.name !== 'id'  && item.name !== 'insertDt' && item.name !== 'updateDt' && item.name !== 'status') {
                arr.push(item);
                this.importExcelData[item.name] = item.defaultValue;
              }
            });
            this.importExcelNav = arr
          })
      ...
      typeNameChange:function(val,row){
        console.log(val);
        this.$api.data.findTableInfo(val)
          .then(res => {
            let dict = res.bean.fieldProps;
            let arr = [];
            dict.forEach(item => {
              if (item.name !== '_id' && item.name !== 'flag') {
                arr.push(item);
              }
            });
            this.tableNavs=arr;
            let dataTemplateForm={};
            arr.forEach(item =>{
              if(item.fieldClass === 'List' &&  item.fieldType === 'CLASS'){
                dataTemplateForm[item.name] = [];
              }else if(item.fieldClass === 'List' && item.size === 0){
                dataTemplateForm[item.name] = new Array(1);
              }else if(item.fieldClass === 'List' && item.size >= 2){
                dataTemplateForm[item.name]=new Array(item.size).fill(null);
              }else{
                dataTemplateForm[item.name]= null;
              }

            })
            if(row === null){
              this.dataTemplateForm = dataTemplateForm;
            }else{
              this.dataTemplateForm = Object.assign(dataTemplateForm,row);
            }

            console.log(this.dataTemplateForm);
            console.log(this.tableNavs);
          })
      ...
      importExcel2(file,fileList){
        this.fileList = fileList.slice(-1);
        console.log(file);
        let fordata = file.raw;
        let file1 = event.currentTarget.files[0];
        this.fordata = new FormData();
        this.fordata.append('file',fordata);
      ...
      BatchAdd:function () {
        ...
        if(this.hasPerms('teach:curriculum:edit')){
          if(this.importMemberIds.length === 0){
            msg.push('选择学生');
            flag = false;
          }
        }else{
          this.importMemberIds[0] = sessionStorage.getItem('userid')
        }
        ...
        this.importExcelNav.forEach(item =>{
          if (that.importExcelData[item.name] === undefined || !that.importExcelData[item.name]) {
            msg.push('输入'+item.zhName);
            flag = false;
          }
        })
        if(flag){
          let datastr={
            ...
            studentId:this.importMemberIds[0],
            experimentId:this.importData.id,
            fieldMapping:this.importExcelData
          };
          this.fordata.append('datastr',JSON.stringify(datastr));
          importOriginResultExcel(that.fordata)
            .then(res =>{
              if(res.retCode === 0){
          ...
          that.$message({message:'请'+msg.toString(),type:'error'})
        }
...
<style>
  .experiment_label .el-form-item__label{
    font-size: 12px;
  }
</style>
